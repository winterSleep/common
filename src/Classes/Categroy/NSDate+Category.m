...
- (NSString *)lunarCalendarWithCurrentDate{
    ...
    NSString *stringDate = [NSString stringWithFormat:@"%@%@",m_str,d_str];
    return stringDate;
}
...
+ (NSDate *)nowDateWithUTCDate:(NSDate *)date
{
    //设置源日期时区
...
- (NSString *)formattedTimeWithFormattedString:(NSString *)formatted{
    NSDateFormatter* formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:formatted];
...
-(NSString *)formattedTime{
    ...
    NSTimeInterval hour = [self hoursAfterDate:date];
...
- (NSString *)formattedDateDescription
{
    ...
    if (timeInterval < 60) {
        ...
        [dateFormatter setDateFormat:@"HH:mm"];
        return [NSString stringWithFormat:@"昨天 %@", [dateFormatter stringFromDate:self]];
    } else if ([[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]] == 86400 * 2) {//昨天
        [dateFormatter setDateFormat:@"HH:mm"];
